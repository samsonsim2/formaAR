...
function TrackedBallPlane({quad,placed,mirrored}){
  ...
  useEffect(()=>{
    ...
    const ball=new THREE.Mesh(new THREE.SphereGeometry(70,48,36),new THREE.MeshStandardMaterial({color:'#287dff',roughness:.3,metalness:.06,transparent:false,depthTest:false,depthWrite:true}));ball.position.z=30;ball.renderOrder=3;scene.add(ball);
    ...
    const displayed={x:el.clientWidth/2,y:el.clientHeight/2,r:90,ready:false};let raf,start=performance.now();const animate=now=>{raf=requestAnimationFrame(animate);const q=quadRef.current;if(q?.length===4){const s=el.clientWidth/1080,base=q.map(p=>({x:p.x*s,y:p.y*s})),targetX=base.reduce((n,p)=>n+p.x,0)/4,targetY=base.reduce((n,p)=>n+p.y,0)/4,targetR=Math.max(72,base.reduce((n,p)=>n+Math.hypot(p.x-targetX,p.y-targetY),0)/4*1.55);if(!displayed.ready){displayed.x=targetX;displayed.y=targetY;displayed.r=targetR;displayed.ready=true}else{displayed.x+=(targetX-displayed.x)*.16;displayed.y+=(targetY-displayed.y)*.16;displayed.r+=(targetR-displayed.r)*.14}const {x:cx,y:cy,r}=displayed,pos=geometry.attributes.position.array,screen=[{x:cx-r,y:cy-r},{x:cx+r,y:cy-r},{x:cx+r,y:cy+r},{x:cx-r,y:cy+r}];screen.forEach((p,i)=>{pos[i*3]=p.x;pos[i*3+1]=p.y;pos[i*3+2]=0});geometry.attributes.position.needsUpdate=true;const bob=Math.abs(Math.sin((now-start)/560))*Math.min(270*s,310),visualScale=Math.min(s*2.1,4.6);ball.scale.setScalar(visualScale);shadow.scale.set(1,.35,1).multiplyScalar(visualScale);ball.position.x=cx;ball.position.y=cy-bob-82*s;shadow.position.x=cx;shadow.position.y=cy;ball.rotation.y+=(now-start)*.000003;plane.visible=ball.visible=shadow.visible=true}else{displayed.ready=false;plane.visible=ball.visible=shadow.visible=false}renderer.render(scene,camera)};animate(start);
...
function App(){
 ...
 const pointInPlacementCircle=(x,y,q,minRadius)=>{if(q.length!==4)return false;const cx=q.reduce((n,p)=>n+p.x,0)/4,cy=q.reduce((n,p)=>n+p.y,0)/4,r=Math.max(minRadius,q.reduce((n,p)=>n+Math.hypot(p.x-cx,p.y-cy),0)/4*1.55);return Math.hypot(x-cx,y-cy)<=r};
