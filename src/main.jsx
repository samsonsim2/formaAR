...
function TrackedBallPlane({quad,placed}){
  ...
  useEffect(()=>{
    ...
    const grid=document.createElement('canvas');grid.width=256;grid.height=256;const gc=grid.getContext('2d');gc.fillStyle='#d8b66d22';gc.fillRect(0,0,256,256);gc.strokeStyle='#f2cb78';gc.lineWidth=3;for(let i=0;i<=256;i+=32){gc.beginPath();gc.moveTo(i,0);gc.lineTo(i,256);gc.stroke();gc.beginPath();gc.moveTo(0,i);gc.lineTo(256,i);gc.stroke()}
    ...
    const plane=new THREE.Mesh(geometry,new THREE.MeshBasicMaterial({map:texture,transparent:true,side:THREE.DoubleSide,depthWrite:false}));scene.add(plane);
    const ball=new THREE.Mesh(new THREE.SphereGeometry(42,32,24),new THREE.MeshStandardMaterial({color:'#ef653b',roughness:.32,metalness:.08}));ball.position.z=30;scene.add(ball);
    const shadow=new THREE.Mesh(new THREE.CircleGeometry(42,32),new THREE.MeshBasicMaterial({color:0x000000,transparent:true,opacity:.28}));shadow.scale.y=.35;shadow.position.z=5;scene.add(shadow);
  ...
  return <div ref={mount} style={{position:'absolute',zIndex:3,inset:0,transform:'scaleX(-1)',pointerEvents:'none',opacity:placed?1:.58,transition:'opacity .2s'}}/>;
}
...
function App(){
 ...
 return <div style={{width:'100vw',height:'100dvh',display:'grid',placeItems:'center',background:'#0d0c0a'}}><main style={{width:'min(100vw,calc(100dvh * 9 / 16))',height:'min(100dvh,calc(100vw * 16 / 9))',aspectRatio:'9 / 16',margin:0,boxShadow:'0 0 70px #000'}}>
   ...
   {!started ? <section className="permission"><div className="cube">◇</div><p className="eyebrow">OPENCV DIAGNOSTIC</p><h1>Find a<br/><em>flat face.</em></h1><p>Aim at a box, book, tabletop, wall, or any visible planar surface.</p><button className="primary" onClick={start}>Start camera <span>→</span></button><small>All processing stays in this browser.</small></section> : <>
    <canvas ref={debugCanvas} style={{position:'absolute',zIndex:2,inset:0,width:'100%',height:'100%',opacity:debugMode!=='off'?1:0,pointerEvents:'none',transform:'scaleX(-1)'}}/>
    <TrackedBallPlane quad={tracking.quad} placed={placed}/>
    <div onClick={e=>{const r=e.currentTarget.getBoundingClientRect(),x=(1-(e.clientX-r.left)/r.width)*1080,y=(e.clientY-r.top)/r.height*1920;if(tracking.quad.length===4&&pointInPlane(x,y,tracking.quad))setPlaced(true)}} style={{position:'absolute',zIndex:4,inset:0,cursor:tracking.quad.length===4?'pointer':'default'}}/>
