...
function useSurfaceTracking(video, debugCanvas, manualPoints, freezeDetection, debugMode, enabled) {
  ...
  useEffect(()=>{
    ...
    let cv,previous,features,raf,stable=0,lastUi=0,lastProcess=0,stopped=false,frameNo=0,lastQuad=[],quadMisses=99,manualLocked=false,lastQuadSeenAt=0;
    ...
    const tick=now=>{
      ...
      if(manualPoints.current.length===4){lastQuad=manualPoints.current.map(p=>({...p}));quadMisses=0;manualLocked=true;lastQuadSeenAt=now;manualPoints.current=[]}
      if(frameNo++%4===0&&!manualLocked&&!freezeDetection.current){
        ...
        if(best.length===4){lastQuad=best;quadMisses=0;lastQuadSeenAt=now}else{quadMisses++;if(now-lastQuadSeenAt>=4000)lastQuad=[]}edges.delete();contours.delete();hierarchy.delete();
      }
      ...
      if(manualLocked){if(homography&&inliers>=8){quadMisses=0;lastQuadSeenAt=now}else quadMisses++;if(now-lastQuadSeenAt>=4000){lastQuad=[];manualLocked=false}}
      if(freezeDetection.current&&!manualLocked){if(homography&&inliers>=8){quadMisses=0;lastQuadSeenAt=now}else quadMisses++;if(now-lastQuadSeenAt>=4000)lastQuad=[]}
...
function TrackedBallPlane({quad,placed,mirrored,ballHitRef}){
  ...
  useEffect(()=>{
    ...
    const ball=new THREE.Mesh(new THREE.SphereGeometry(70,48,36),new THREE.MeshStandardMaterial({color:'#287dff',roughness:.3,metalness:.06,transparent:false,opacity:1,blending:THREE.NoBlending,depthTest:false,depthWrite:true}));ball.position.z=30;ball.renderOrder=3;scene.add(ball);
    ...
    const displayed={x:el.clientWidth/2,y:el.clientHeight/2,rx:90,ry:58,angle:0,ready:false};let raf,start=performance.now(),lastFrame=0;const animate=now=>{raf=requestAnimationFrame(animate);if(now-lastFrame<1000/30)return;lastFrame=now;const q=quadRef.current;if(q?.length===4){const s=el.clientWidth/1080,base=q.map(p=>({x:p.x*s,y:p.y*s})),targetX=base.reduce((n,p)=>n+p.x,0)/4,targetY=base.reduce((n,p)=>n+p.y,0)/4,edges=base.map((p,i)=>{const n=base[(i+1)%4];return{length:Math.hypot(n.x-p.x,n.y-p.y),angle:Math.atan2(n.y-p.y,n.x-p.x)}}),pair0=(edges[0].length+edges[2].length)/2,pair1=(edges[1].length+edges[3].length)/2,major=pair0>=pair1?pair0:pair1,minor=pair0>=pair1?pair1:pair0,targetAngle=pair0>=pair1?edges[0].angle:edges[1].angle,targetRx=Math.max(72,major*.78),targetRy=Math.max(targetRx*.38,Math.min(targetRx,minor*.78));if(!displayed.ready){Object.assign(displayed,{x:targetX,y:targetY,rx:targetRx,ry:targetRy,angle:targetAngle,ready:true})}else{displayed.x+=(targetX-displayed.x)*.24;displayed.y+=(targetY-displayed.y)*.24;displayed.rx+=(targetRx-displayed.rx)*.2;displayed.ry+=(targetRy-displayed.ry)*.2;displayed.angle+=Math.atan2(Math.sin(targetAngle-displayed.angle),Math.cos(targetAngle-displayed.angle))*.2}const {x:cx,y:cy,rx,ry,angle}=displayed,c=Math.cos(angle),sn=Math.sin(angle),corners=[[-rx,-ry],[rx,-ry],[rx,ry],[-rx,ry]],pos=geometry.attributes.position.array;corners.forEach(([x,y],i)=>{pos[i*3]=cx+x*c-y*sn;pos[i*3+1]=cy+x*sn+y*c;pos[i*3+2]=0});geometry.attributes.position.needsUpdate=true;const bob=Math.abs(Math.sin((now-start)/560))*Math.min(270*s,310),visualScale=Math.min(s*2.1,4.6),ballY=cy-bob-82*s;ball.scale.setScalar(visualScale);shadow.scale.set(1,.35,1).multiplyScalar(visualScale);ball.position.x=cx;ball.position.y=ballY;shadow.position.x=cx;shadow.position.y=cy;ball.rotation.y+=(now-start)*.000003;ballHitRef.current={x:cx/s,y:ballY/s,r:70*visualScale/s};plane.visible=ball.visible=shadow.visible=true}else{displayed.ready=false;ballHitRef.current=null;plane.visible=ball.visible=shadow.visible=false}renderer.render(scene,camera)};animate(start);
  ...
  return <div ref={mount} style={{position:'absolute',zIndex:3,inset:0,transform:mirrored?'scaleX(-1)':'none',pointerEvents:'none',opacity:1}}/>;
}
...
function App(){
 const video=useRef(null),debugCanvas=useRef(null),manualPoints=useRef([]),freezeDetection=useRef(false),debugModeRef=useRef('off'),ballHitRef=useRef(null),[debugMode,setDebugMode]=useState('off'),[placed,setPlaced]=useState(false),[started,setStarted]=useState(false),[cameraError,setCameraError]=useState(''),[cameraResolution,setCameraResolution]=useState(''),[mirrored,setMirrored]=useState(false);
 ...
 const pointInPlacementTarget=(x,y,q,minRadius)=>{const ball=ballHitRef.current;if(ball&&Math.hypot(x-ball.x,y-ball.y)<=ball.r)return true;if(q.length!==4)return false;const cx=q.reduce((n,p)=>n+p.x,0)/4,cy=q.reduce((n,p)=>n+p.y,0)/4,edges=q.map((p,i)=>{const n=q[(i+1)%4];return{length:Math.hypot(n.x-p.x,n.y-p.y),angle:Math.atan2(n.y-p.y,n.x-p.x)}}),pair0=(edges[0].length+edges[2].length)/2,pair1=(edges[1].length+edges[3].length)/2,major=pair0>=pair1?pair0:pair1,minor=pair0>=pair1?pair1:pair0,angle=pair0>=pair1?edges[0].angle:edges[1].angle,rx=Math.max(minRadius,major*.78),ry=Math.max(rx*.38,Math.min(rx,minor*.78)),dx=x-cx,dy=y-cy,c=Math.cos(-angle),s=Math.sin(-angle),localX=dx*c-dy*s,localY=dx*s+dy*c;return localX*localX/(rx*rx)+localY*localY/(ry*ry)<=1};
...
<video ref={video} className={`camera-feed${mirrored?' camera-feed--mirrored':''}`} muted playsInline/>
   ...
   {!started ? <section className="permission"><div className="cube">◇</div><p className="eyebrow">OPENCV DIAGNOSTIC</p><h1>Find a<br/><em>flat face.</em></h1><p>Aim at a box, book, tabletop, wall, or any visible planar surface.</p><button className="primary" onClick={start}>Start camera <span>→</span></button><small>All processing stays in this browser.</small></section> : <>
    ...
    <TrackedBallPlane quad={tracking.quad} placed={placed} mirrored={mirrored} ballHitRef={ballHitRef}/>
    <div onClick={e=>{const r=e.currentTarget.getBoundingClientRect(),screenX=(e.clientX-r.left)/r.width,x=(mirrored?1-screenX:screenX)*1080,y=(e.clientY-r.top)/r.height*1920;if(pointInPlacementTarget(x,y,tracking.quad,72*1080/r.width))setPlaced(true)}} style={{position:'absolute',zIndex:4,inset:0,cursor:tracking.quad.length===4?'pointer':'default',WebkitTapHighlightColor:'transparent',touchAction:'manipulation',userSelect:'none'}}/>
    ...
    <div style={{position:'absolute',zIndex:6,left:18,right:18,bottom:24,padding:'13px 15px',background:'#111c',border:'1px solid #ffffff25',fontSize:10,lineHeight:1.6,letterSpacing:'.08em',pointerEvents:'none'}}><b style={{color:placed?'#76e6a5':'#ffcc66'}}>{placed?'● PLACED — TRACKING ACTIVE':'● PREVIEW — TAP THE RING TO PLACE'}</b><br/><span style={{color:'#aaa'}}>{debugMode!=='off'?`${tracking.points.length} features · ${tracking.quad.length===4?'plane candidate':'no quadrilateral'} · light ${tracking.diagnostics.brightness}/255 · contrast ${tracking.diagnostics.contrast} · texture ${tracking.diagnostics.texture}`:'9:16 crop · CV 1080 × 1920 · 4s tracking-loss hold'}</span></div>
   </>}
 </main></div>
}
